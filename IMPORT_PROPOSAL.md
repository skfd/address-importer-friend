...
- **Tooling:** [toronto-2-address-import](https://github.com/skfd/toronto-2-address-import) (this repo) + [toronto-addresses-import](https://github.com/skfd/toronto-addresses-import) (upstream scraper)
- **Pilot evidence:** [pilot run #15](https://skfd.github.io/toronto-2-address-import/pilot/runs/15/) — read-only snapshot of one completed pilot run (tile `high-park-swansea-sw-se`, 250 candidates) rendered in the full review UI.
- **Sample dev-sandbox upload with errors:** [changeset 616162](https://api06.dev.openstreetmap.org/changeset/616162) — one upload generated by the import tool against `master.apis.dev.openstreetmap.org`, evidence the upload mechanics work end-to-end.
- **Sample of manual revert:** [changeset 617602](https://api06.dev.openstreetmap.org/changeset/617602)
- **Sample of good changeset:** [changeset 617603](https://api06.dev.openstreetmap.org/changeset/617603)
- **Live status:** tracked in the [project README](https://github.com/skfd/toronto-2-address-import#status).
...
**Geographic scope:** the City of Toronto, Ontario — OSM admin boundary [relation 324211](https://www.openstreetmap.org/relation/324211), covering the post-1998 amalgamated city (former municipalities of Toronto, East York, Etobicoke, North York, Scarborough, and York). The 1,297 review tiles named in §3 partition this boundary.
...
- **Phase 2 — ward-level rollout.** Proceed ward-by-ward, working through the 1,297 tiles one at a time. Retain manual approval of a random sample (≥5%) even for auto-approvable items.
...
| Tag | Value |
|---|---|
| `comment` | `Toronto Open Data address import, run=<run_name>` (template in `config.toml`) |
| `source` | `City of Toronto Open Data` |
| `import` | `yes` |
| `bot` | `no` |
| `created_by` | `t2-address-import` |
| `import:client_token` | random per-run UUID — used only for server-side idempotent retry after a network failure; looked up before reopening a changeset so a dropped connection never results in two parallel uploads of the same run |
...
| Rate pressure on OSM API / planet feed. | Operator-triggered cadence targeting ≤1 changeset/min; one run per changeset, with each run sized to a 250–750-address tile (§3); pilot tile first; no parallel uploaders from this tool. |
...
| Community unaware of ongoing import. | Wiki page kept updated with per-run progress; changeset comments include run name; contact email published. |
...
- **Routine revert (one bad changeset).** A changeset later identified as problematic is reverted using the [JOSM Reverter plugin](https://wiki.openstreetmap.org/wiki/JOSM/Plugins/Reverter). The changeset id is available in the audit log for every uploaded candidate, and the published OSM-ID list (§7 post-upload reconciliation) groups candidates by changeset for quick lookup.
...
1. **Changeset comment template.** Current format is `Toronto Open Data address import, run=<run_name>`. Any information we should add or remove?
2. **Post-import monitoring.** How long after the final upload should we commit to watching for community-raised issues? Proposing 90 days.
...
- OSM imports mailing list archive: <https://lists.openstreetmap.org/pipermail/imports/>
